...
class SGLANGwithChatTemplate:
    ...
    def __init__(
        ...
        assert Engine is not None, 'sglang not found.'
        ...
        self._load_model(path, model_kwargs, self.max_seq_len)
    ...
    def _load_model(self, path: str, added_model_kwargs: dict = dict(), max_seq_len: int = None):
        ...
        # Set context_length if max_seq_len is provided
        if max_seq_len is not None and 'context_length' not in model_kwargs:
            model_kwargs['context_length'] = max_seq_len
            self.logger.info(f'Setting SGLang context_length to {max_seq_len}')

        self.logger.info(f'SGLang Engine model_kwargs: {model_kwargs}')
        self.engine = Engine(model_path=path, **model_kwargs)
    ...
    def generate(self, inputs: List[str], max_out_len: int, stopping_criteria: List[str] = [], **kwargs) -> List[str]:
        ...
        print(f"  Sending {len(messages)} requests to SGLang engine (batch_size=32)...")

        import tqdm
        outputs = []
        chunk_size = 32
        for i in tqdm.tqdm(range(0, len(messages), chunk_size), desc="Generating"):
            batch_outputs = self.engine.generate(messages[i:i + chunk_size], sampling_kwargs)
        ...
        output_strs = [o['text'] for o in outputs]
        return output_strs
...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default='/user/linbiyuan/models/MiniCPM-SALA')
...
def main():
    ...
    model = SGLANGwithChatTemplate(
        ...
        model_kwargs={
            "tp_size": int(os.environ.get('GPU_PER_WORKER', 1)),
            "trust_remote_code": True,
    ...
    mcq_tasks = ['mcq', 'expert_mcq']
    long_context_tasks = [
        'niah', 'cwe', 'fwe', 'qa', 'vt', 'lcx',
        'needle_in_haystack', 'common_words_extraction', 'freq_words_extraction', 'context_qa', 'variable_tracking', 'long_context_mix'
    ]
